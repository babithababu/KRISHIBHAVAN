...
/*	$email=$_SESSION["user"];
					$s=mysqli_query($con,"select *from tblfarmer where username='$email'");
					$id=mysqli_fetch_assoc($s);
						//$id=$row[0];*/
					
$tt="SELECT  dob,CURDATE(),DATEDIFF(CURDATE(),dob) FROM tblfarmer where DATEDIFF(CURDATE(),dob) >65 and farmerid=$id";
$ttt=mysqli_query($con,$tt);
	
	
	$num_row=mysqli_num_rows($ttt);
	if($num_row>0)
	{
?>			
			<form method="POST" action="" enctype="multipart/form-data">
						...
						cellspacing="2px" border="1" cellspacing="10">

						<tr>
						<td colspan=2>
						<center><font size=4><b>KARSHIKA PENSION</b></font></center>
						</td>
						</tr>
						<tr><td colspan=2>
						...
						<tr><tr><td><br><td></tr>
						<td>Reg id:</td>
						<td align="left"><input type="text" name="name" class="mm" size="20" value="<?php echo $id;?>"></td>
						</tr>
						...
						<tr><th>Attach the Following</th></tr>
						<tr>
						...
						<td colspan="2"><input type="submit" value="Submit Form" name="submit"/></td>
								...
								include("connection.php");
										...
										if( $_FILES['file2']['name'] != "" )
										{
											...
											{
												echo "There was an error uploading age proof, please try again!";
											}
											...
											/* $rr="select dob from tblfarmer where farmerid=$farid";
											$www=mysqli_query($con,$rr);
											echo $rr;
											echo $apldate;
											while($row=mysqli_fetch_array($www))
													{
														$datee=$row["dob"];						
													}
													echo $datee;
													$diff=date_diff($apldate,$datee);
													echo $diff;
											 */
											
											$s= "INSERT INTO `tblpension`(`farmerid`, `incomecert`, `taxcert`, `ageproof`, `date`) VALUES($farid,'$file','$filee','$fileee','$apldate')";
											mysqli_query($con,$s);
											echo $s;
											echo "success";
	...
	{		
		echo "<script>alert('You are not elligible');  window.location='userprofile.php';</script>";
	}
	?>
		<!-- End Footer -->
