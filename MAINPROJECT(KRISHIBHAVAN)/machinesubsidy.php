...
<tr>
<td colspan=2>
<center><font size=4><b>Aggriculture Machines Subsidy</b></font></center>
</td>
</tr>
<tr><td colspan=2>
...
<td align="left"><select name="machine" width="15px">
        <?php
       $sss=mysqli_query($con,"select *from tblsubsidy where category='aggriculture machine'");
...
<td align="left"><input type="text" name="no" class="mm" size="15"></td>
</tr>
<tr>
<td align="left">Purchase Date:</td>
<td align="left"><input type="date" name="purdate" class="mm" size="15"></td>
</tr>
...
include("connection.php");
		...
		if( $_FILES['file']['name'] != "" )
		{
			...
			$tt=DATEDIFF(CURDATE(),$purdate);
			echo $tt;
/* $ttt=mysqli_query($con,$tt);
if ($ttt<31)	
{	
			$s="INSERT INTO `tblmachinesubsidy`(`farid`, `machine`, `amount`, `purdate`, `apldate`, `bill`) VALUES($id,'$machine',$no,'$purdate','$apldate','$file')";
			mysqli_query($con,$s);
			echo $s;
			echo "success";
}
...
{
	echo "<script>alert('You are not elligible');  window.location='machinesubsidy.php';</script>";
}
	*/		
	} 
		?>
